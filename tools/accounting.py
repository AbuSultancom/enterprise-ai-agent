...
# ─── Text-to-SQL: answer ANY accounting question in natural language ─────

_TEXT2SQL_SYSTEM = """You are an expert {dialect} SQL writer for an ERP accounting system.
...
- Use ONLY the tables and columns listed in the schema below — never invent names.
- Use {dialect} syntax for dates and row limits (Oracle: FETCH FIRST 200 ROWS ONLY; MySQL/SQLite/PostgreSQL: LIMIT 200; SQL Server: SELECT TOP 200).
- If invoices have a status column, prefer posted/approved rows for business-result questions.
- For date ranges, today's date is {today}. "this month" = 1st of current month until today.
...
def _extract_sql(text: str) -> str:
    ...
    m = _re.search(r"```(?:sql)?\s*(.*?)```", text, _re.DOTALL | _re.IGNORECASE)
    sql = m.group(1) if m else text
    return sql.strip().rstrip(";").strip()


@registry.register(
    description="Answer ANY accounting/ERP question in natural language (Arabic or English) by writing "
    "and running a safe read-only SQL query against the configured database. Use this when the fixed "
    "accounting queries do not cover the question (e.g. 'who is our best customer this quarter?', "
    "'average invoice value last week', 'how many invoices contain item X?').",
    parameters={
        "question": {
            "type": "str",
            "description": "The business question to answer from the ERP database",
        },
        "database": {
            "type": "str",
            "description": "Database key (optional; uses default if empty)",
...
async def ask_database(question: str, database: str = "") -> str:
    db = database or None
    try:
        schema_text = connector.describe_schema(db)
        dialect = connector.db_dialect(db)
    except Exception as e:
        return f"⚠️ Accounting database not available: {e}"

    try:
        from llm_gateway.gateway import Message
    except ImportError:
        return "❌ LLM gateway not available."

    import datetime as _dt

    system = _TEXT2SQL_SYSTEM.format(
        dialect=dialect, schema=schema_text, today=_dt.date.today().isoformat()
    )
    gw = _get_llm()
    messages = [
    ...
    last_err = ""
    for _attempt in (1, 2):  # one self-correction retry
        try:
            resp = await gw.chat(messages, mask_pii=False)
            sql = _extract_sql(resp.content)
        except Exception as e:
            return f"❌ SQL generation failed: {e}"
        try:
            rows = connector.run_sql(sql, db_name=db)
            return _to_json(
                {
                    "question": question,
                    "sql": sql,
                    "row_count": len(rows),
                    "rows": rows[:50],
                    "note": "Summarize these rows for the user; lead with the headline number.",
                }
            )
        except (PermissionError, RuntimeError, ValueError) as e:
            last_err = str(e)
            messages.append(Message(role="assistant", content=sql))
            messages.append(
                Message(
                    role="user",
                    content=(
                        f"This SQL failed with: {last_err}\n"
                        "Write a corrected SELECT that avoids this error. Output ONLY the SQL."
                    ),
                )
            )
    return f"❌ Could not answer from the database after 2 attempts. Last error: {last_err}"
